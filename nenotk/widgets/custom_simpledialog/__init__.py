"""
# Purpose
Provides simple, consistent dialogs for string, integer, float, combo, radio, yes/no, info, progress, and path confirmation.

## API
- `askstring(title, prompt, initialvalue=None, detail=None, parent=None, icon_image=None) -> Optional[str]`
    - Prompt for a string value; returns entered string or None if cancelled.
- `askinteger(title, prompt, initialvalue=None, minvalue=None, maxvalue=None, detail=None, parent=None, icon_image=None) -> Optional[int]`
    - Prompt for integer input with optional min/max; returns int or None.
- `askfloat(title, prompt, initialvalue=None, minvalue=None, maxvalue=None, detail=None, parent=None, icon_image=None) -> Optional[float]`
    - Prompt for float input with optional min/max; returns float or None.
...
- `showprogress(title, prompt, task_function, args=(), kwargs=None, max_value=100, parent=None, icon_image=None, auto_close=True) -> Any`
    - Runs a background task with progress bar; returns task result or None if cancelled/error.
- `confirmpath(title, prompt, path, detail=None, parent=None, icon_image=None) -> tuple[Optional[bool], Optional[str]]`
    - Confirm or change a file path; returns (True, path), (False, None), or (None, None).

## Notes
- All dialogs return None or a default value if cancelled.
- If no parent is provided, a temporary root window is created and destroyed automatically.
- askradio supports choices as strings or tuples (value, label[, detail]).
- showprogress runs the task in a background thread and supports cancellation via InterruptedError.

## Example
```python
name = askstring("Name", "Enter your name:", initialvalue="Alice", detail="Used for personalization.")
count = askinteger("Count", "Enter a number:", initialvalue=5, minvalue=1, maxvalue=10)
confirmed, path = confirmpath("Confirm Output", "Save to:", "C:\\output")
```
"""
