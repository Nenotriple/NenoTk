"""
# Purpose
Provides a custom Tkinter widget for advanced find and replace operations in a `tk.Text` widget.

## API
- Class: `FindReplaceEntry(parent: tk.Frame, text_widget: tk.Text, ...)`
    - A frame widget for searching and replacing text.
    - Methods:
        - `show_widget()`: Show the widget and search for selected text.
        - `hide_widget()`: Hide the widget and clear highlights.
        - `get_selected_text() -> str`: Get selected text from the associated text widget.
        - `toggle_replace_row()`: Show/hide the replace row.
        - `search_for_text(text: str)`: Set find to the provided string and search.
        - `perform_search(event=None)`: Perform search based on current entry.
        - `replace_current()`: Replace the current match.
        - `replace_all()`: Replace all matches.

- Class: `TextSearchManager(text_widget: tk.Text)`
    - Manages search, navigation, highlighting, and replacement logic.
    - Methods:
        - `find_all(search_term: str, case_sensitive: bool, match_whole_word: bool, use_regex: bool) -> int`
        - `next_match()`
        - `prev_match()`
        - `clear_highlights()`
        - `replace_current(replacement: str) -> bool`
        - `replace_all(search_term: str, replacement: str, case_sensitive: bool, match_whole_word: bool, use_regex: bool) -> int`

## Notes
- The widget can be shown/hidden via keyboard shortcuts (Ctrl+F, Esc).
    - `parent` must use grid layout for the `FindReplaceEntry` to show/hide correctly.

## Example
```python
from nenotk import FindReplaceEntry

frame = ttk.Frame(root)
text = tk.Text(frame)
find_replace = FindReplaceEntry(frame, text)
find_replace.grid(row=0, column=0, sticky="ew")
find_replace.grid_remove()
text.bind("<Control-f>", find_replace.show_widget)
```
"""
...
if __name__ == "__main__":
    ...
    # Step 1: Create a frame to hold the text and FindReplaceEntry widget.
    # You must use grid layout for show and hide logic to work correctly.
    frame = ttk.Frame(root, padding=10)
    frame.grid(row=0, column=0, sticky="nsew")
    frame.columnconfigure(0, weight=1)
    frame.rowconfigure(1, weight=1)

    # Step 2: Create and position a text widget in the frame.
    text = tk.Text(frame, wrap="none")
    text.grid(row=1, column=0, sticky="nsew")
    text.insert("end", "This is a test. Test the Find and Replace widget by pressing Ctrl+F.\nTry searching for 'test'.")

    # Step 3: Create and position a FindReplaceEntry widget in the frame.
    # Use grid_remove() to hide the FindReplaceEntry initially.
    find_replace = FindReplaceEntry(frame, text)
    find_replace.grid(row=0, column=0, sticky="ew")
    find_replace.grid_remove()

    # Step 4: Setup key bindings for the text widget
    # If you used grid layout, you can directly bind the show_widget method.
    # For live updates on key release, bind perform_search as well.
    # The FindReplaceEntry widget also supports hiding itself with the Escape key when in focus.
    # You can also bind Escape on the text widget to hide it when focus is there.
    text.bind("<Control-f>", find_replace.show_widget)
    text.bind("<KeyRelease>", find_replace.perform_search)
    text.bind("<Escape>", find_replace.hide_widget)
