...
- Class: `PopUpZoom(widget: Label)`
    - set_image(image: Image.Image) -> None
        - Set the image to be used for zooming.
    - show_popup(event: Event) -> None
        - Show the popup window and update its image based on mouse position.
    - hide_popup(event: Event) -> None
        - Hide the popup window.
    - zoom_enabled: BooleanVar
        - Controls whether zoom is enabled.
...
## Example
```
from nenotk import PopUpZoom
from tkinter import Label
from PIL import Image

label = Label(root)
zoom = PopUpZoom(label)
