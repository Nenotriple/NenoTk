"""
Tooltip widget for tkinter: attach rich, configurable tooltips to any widget.

Supported values:
- `Tk.Label Arguments`: `bg`, `fg`, `padx`, `pady`, ... (see defaults)
- `text`: string or list of strings (for multi-line tooltips)
- `opacity`: float 0.0 - 1.0
- `origin`: `"mouse"` or `"widget"` (positioning reference)
- `widget_anchor` / `tooltip_anchor`: `"n", "ne", "e", "se", "s", "sw", "w", "nw", "center"`
- `follow_mouse`: bool
- `show_delay` / `hide_delay`: int milliseconds
- `animation`: `"fade"`, `"slide"`, or `"none"`
- `anim_in` / `anim_out`: int milliseconds


Public API:
- ToolTip.bind(widget, **kwargs)
- ToolTip.configure(**kwargs)
- ToolTip.unbind()
- ToolTip.hide()
"""
#region Imports
...
class ToolTip:
    ...
    @classmethod
    def bind(cls: ToolTip, widget: Widget, **kwargs: Any) -> 'ToolTip':
        """Binds a tooltip for widget; kwargs limited to PARAMS."""
        return cls(widget, **kwargs)


    @classmethod
    def create(cls: ToolTip, widget: Widget, **kwargs: Any) -> 'ToolTip':
    ...
    def _cancel_auto_hide(self) -> None:
        ...
        if self.hide_id:
            try:
                ...
                print(f"ERROR: ToolTip._cancel_auto_hide() - {e}")
                pass
            self.hide_id = None
    ...
    def _build_tooltip_content(self, parent: Toplevel) -> None:
        ...
        try:
            ...
            print(f"ERROR: ToolTip._build_tooltip_content() - {e}")
    ...
    def _get_text(self) -> Union[str, list[str]]:
        ...
        if callable(self.text):
            try:
                ...
                print(f"ERROR: ToolTip._get_text() - {e}")
                return ""
        else:
            value = self.text
